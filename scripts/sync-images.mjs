...
const modelOutDir = path.join(root, 'public/media/models/auto');
...
const incomingModelSources = [
  {
    dir: path.join(root, 'incoming/models'),
    sourcePrefix: 'incoming/models',
    page: 'model',
    category: '3d-model',
  },
];
...
await Promise.all([
  ...
  mkdir(modelOutDir, { recursive: true }),
  ...
  ...incomingModelSources.map((source) =>
    mkdir(source.dir, { recursive: true }),
  ),
]);
...
for (const source of incomingModelSources) {
  imported += await syncModelSource(source);
}

if (imported === 0) {
  console.log('No incoming images, models, or manifests found.');
  process.exit(0);
}
...
async function syncModelSource(source) {
  const files = (await readdir(source.dir, { withFileTypes: true }))
    .filter((entry) => entry.isFile())
    .map((entry) => entry.name)
    .filter((name) => path.extname(name).toLowerCase() === '.glb')
    .sort((a, b) => a.localeCompare(b));
  ...
  for (const file of files) {
    const sourcePath = path.join(source.dir, file);
    const modelBuffer = await readFile(sourcePath);
    const metadata = await readSidecar(source.dir, file);
    const checksum = createHash('sha256')
      .update(modelBuffer)
      .update(JSON.stringify(metadata))
    ...
    const updatedAt = toDateOnly(sourceStat.mtime);
    const title = parsed.title;
    const outputName = `${slug}.glb`;
    const contentPath = path.join(generatedContentDir, `${slug}.mdx`);

    await copyFile(sourcePath, path.join(modelOutDir, outputName));
    ...
    if (await shouldWriteMdx(contentPath, checksum)) {
      await writeFile(
        ...
        makeMdx({
          ...
          date,
          updatedAt,
          thumbnail: cleanString(metadata.thumbnail),
          thumbnailAlt:
            cleanString(metadata.thumbnailAlt) ||
            `Interactive 3D model: ${title}.`,
          heroImage: cleanString(metadata.heroImage),
          heroImageAlt:
            cleanString(metadata.heroImageAlt) ||
            `Interactive 3D model: ${title}.`,
          modelUrl: `/media/models/auto/${outputName}`,
          aiTool: cleanString(metadata.aiTool),
          tools: normalizeList(metadata.tools, metadata.aiTool),
          tags: normalizeTags(
            metadata.tags,
...
function parseSourceName(file, metadata = {}) {
  ...
  const sourceTitle = cleanString(metadata.sourceName);
  const sourceBase = sourceTitle
    ? path.basename(sourceTitle, path.extname(sourceTitle))
    : '';
  ...
  const title = rawTitle || titleize(sourceBase || nameWithoutDate);
...
function validPage(page, fallback) {
  return page === 'content' || page === 'design' || page === 'model'
    ? page
    : fallback;
}
